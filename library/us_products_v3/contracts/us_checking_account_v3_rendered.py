...
#    us_checking_account_v3.py
# md5:d0d1dc112353beb1e0adb7dc9c721583
...
def execution_schedules() -> list[tuple[str, dict[str, Any]]]:
    ...
    interest_application_frequency = utils_get_parameter(
...
def scheduled_code(event_type: str, effective_date: datetime) -> None:
    if event_type == "ACCRUE_INTEREST_AND_DAILY_FEES":
        ...
        deposit_interest_application_frequency = utils_get_parameter(
            vault, "deposit_interest_application_frequency", union=True
        )
        ...
        vault.update_event_type(
            event_type="APPLY_ACCRUED_DEPOSIT_INTEREST",
            schedule=utils_create_event_type_schedule_from_schedule_dict(new_schedule),
        )
...
def pre_parameter_change_code(
    ...
    if "daily_atm_withdrawal_limit" in parameters:
        max_atm = utils_get_parameter(
            vault, name="maximum_daily_atm_withdrawal_limit", is_json=True
        )
        tier_names = utils_get_parameter(vault, name="tier_names", is_json=True)
        this_account_flags = utils_get_active_account_flags(vault, tier_names)
        max_atm = utils_get_dict_value_based_on_account_tier_flag(
...
@requires(parameters=True, flags=True, balances="latest live", postings="1 day")
def pre_posting_code(postings: PostingInstructionBatch, effective_date: datetime) -> None:
    ...
    main_denomination = utils_get_parameter(vault, "denomination")
    additional_denominations = utils_get_parameter(vault, "additional_denominations", is_json=True)
    ...
    if unallowed_denominations:
        raise Rejected(
            f"Postings received in unauthorised denominations {', '.join(unallowed_denominations)}. Authorised denominations are {', '.join(accepted_denominations)}",
    ...
    for denomination in posting_denominations:
        available_balance = utils_get_available_balance(balances, denomination)
        proposed_delta = utils_get_available_balance(postings.balances(), denomination)
        if denomination == main_denomination:
            standard_overdraft_limit = utils_get_parameter(vault, "standard_overdraft_limit")
            ...
            raise Rejected(
                f"Postings total {denomination} {proposed_delta}, which exceeds the available balance of {denomination} {available_balance}",
                reason_code=RejectedReason.INSUFFICIENT_FUNDS,
            )
...
def post_posting_code(postings: PostingInstructionBatch, effective_date: datetime) -> None:
    main_denomination = utils_get_parameter(name="denomination", vault=vault)
    autosave_rounding_amount = utils_get_parameter(name="autosave_rounding_amount", vault=vault)
    autosave_savings_account = utils_get_parameter(name="autosave_savings_account", vault=vault)
...
def utils_common_get_transaction_type(
    instruction_details: dict[str, str], txn_code_to_type_map: dict[str, str], default_txn_type: str
) -> str:
    """
...
utils_get_transaction_type = utils_common_get_transaction_type
...
def utils_get_active_account_flags(vault: Any, interesting_flag_list: list[str]) -> list[str]:
    """
...
def utils_get_daily_schedule(vault: Any, param_prefix: str, event_type: str) -> utils_EventTuple:
    """
...
def utils_create_event_type_schedule_from_schedule_dict(schedule_dict: dict) -> EventTypeSchedule:
    """
...
def utils_falls_on_calendar_events(
    vault: Any, localised_effective_date: datetime, calendar_events: CalendarEvents
) -> bool:
    """
...
def utils_get_dict_value_based_on_account_tier_flag(
...
event_types = [
    ...
    EventType(name="APPLY_MONTHLY_FEES", scheduler_tag_ids=["US_CHECKING_APPLY_MONTHLY_FEES_AST"]),
]
parameters = [
    Parameter(
        name="fee_free_overdraft_limit",
        level=Level.INSTANCE,
        description="The borrowing limit for the main denomination agreed between the bank and the customer, before a fee is applied. Setting to 0 will disable this feature.",
    ...
    Parameter(
        ...
        level=Level.INSTANCE,
        description="Standard Overdraft is the maximum borrowing limit for the main denomination on the account. Withdrawals that breach this limit will be rejected.",
    ...
    Parameter(
        ...
        level=Level.INSTANCE,
        description="Daily amount from the main denomination that can be withdrawn by ATM. This must less than or equal to the maximum daily ATM withdrawal limit. If set to -1 this is treated as unlimited and overrides the maximum.",
    ...
    Parameter(
        ...
        description="The main currency in which the product operates. The following features will only be available for the main denomination: Overdraft, deposit interest, minimum account balance and other fees. Contract defined limitations will also only apply to postings made in this currency.",
    ...
    Parameter(
        ...
        shape=StringShape,
        level=Level.TEMPLATE,
        description="Currencies that are accepted for this product, formatted as a json list of currency codes. Transactions in these currencies will act as simple pots, and will not have an overdraft or other features available to them.",
    ...
    Parameter(
        ...
        shape=StringShape,
        level=Level.TEMPLATE,
        description="Tiered interest rates applicable to the main denomination as determined by the tier ranges. This is the gross interest rate (per annum) used to calculate interest on customers deposits. This is accrued daily and applied according to the schedule.",
    ...
    Parameter(
        ...
        level=Level.TEMPLATE,
        description="The per-transaction fee charged for any transaction that uses the standard overdraft.",
    ...
    Parameter(
        ...
        level=Level.TEMPLATE,
        description="A monthly cap on accumulated standard overdraft daily fee and standard overdraft per-transaction fee for entering an overdraft. If set to 0 this is treated as unlimited.",
    ...
    Parameter(
        ...
        level=Level.TEMPLATE,
        description="The fee charged per savings sweep transaction. Applicable only when the account is setup for savings sweep.",
    ...
    Parameter(
        ...
        level=Level.TEMPLATE,
        description="A daily cap on how many times savings sweep fee can be charged to the customer. If set to -1 this is treated as unlimited. Applicable only when the account is setup for savings sweep.",
    ...
    Parameter(
        ...
        level=Level.TEMPLATE,
        description="The multiple that funds can be swept in for savings sweep. If set to 50, funds are transferred from savings account(s) in multiples of 50 only. If set to 0, the exact amount required (or available) is transferred.",
    ...
    Parameter(
        ...
        level=Level.TEMPLATE,
        description='The days in the year for interest accrual calculation. Valid values are "actual", "365", "360"',
    ...
    Parameter(
        ...
        level=Level.TEMPLATE,
        description="The monthly fee charged for account maintenance for each account tier. This fee can be waived if any of the following criteria are satisfied: 1) The minimum daily balance threshold is satisfied for the period 2) The account has been added to a supervisor plan and the minimum combined balance threshold is satisfied across plan accounts for the period 3) The minimum deposit threshold is satisfied for the period",
    ...
    Parameter(
        ...
        level=Level.TEMPLATE,
        description="The promotional monthly fee charged for account maintenance for each account tier. This fee can be waived subject to the same criteria as normal monthly maintenance fee.",
    ...
    Parameter(
        ...
        level=Level.TEMPLATE,
        description="The minimum daily balance required for each account tier. If the mean daily balance in the main denomination falls below this, the monthly maintenance fee will be charged if no other waive criteria are satisfied. The calculation is performed every month on the anniversary of the account opening day, for each day since the last calculation. It takes samples of the balance at the fee application time. If set a minimum balance fee will also be applied.",
    ...
    Parameter(
        ...
        level=Level.TEMPLATE,
        description=" Only applicable if the account has been added to a supervisor plan. The minimum combined daily checking and savings account balance required for each account tier. If the mean combined daily balance in the main denomination falls below this, the monthly maintenance fee will be charged if no other waive criteria are satisfied. The calculation is performed every month, for each day since the last calculation. It takes samples of the balances at the fee application time.",
    ...
    Parameter(
        ...
        level=Level.TEMPLATE,
        description="The minimum deposit amount required for each account tier. If the minimum  deposit is reached the monthly maintenance fee will not be charged.",
    ...
    Parameter(
        ...
        level=Level.TEMPLATE,
        description="The monthly fee charged while the account is inactive. While inactive this fee replaces other minimum balance and periodic maintenance fees.",
    ...
    Parameter(
        ...
        level=Level.TEMPLATE,
        description="Maximum daily amount from the main denomination that can be withdrawn by ATM. If daily_atm_withdrawal_limit is set to -1 however then it is unlimited.",
    ...
    Parameter(
        ...
        level=Level.TEMPLATE,
        description="For any given spend with the primary denomination, this is the figure to round up to: the nearest multiple higher than the purchase amount. Only used if autosave_savings_account is defined.",
    ...
    Parameter(
        ...
        level=Level.TEMPLATE,
        description="The transaction types that are covered by the standard overdraft if enabled with STANDARD_OVERDRAFT_TRANSACTION_COVERAGE flag. If a customer does not opt into standard overdraft transaction coverage, then transactions of these types will be rejected if standard overdraft is required for them. All other transaction types can use standard overdraft.",
...
def _charge_overdraft_per_transaction_fee(
    ...
    overdraft_per_transaction_fee = utils_get_parameter(
        vault, name="standard_overdraft_per_transaction_fee"
    )
    fee_free_overdraft_limit = utils_get_parameter(vault, name="fee_free_overdraft_limit")
    overdraft_fee_income_account = utils_get_parameter(vault, name="overdraft_fee_income_account")
    standard_overdraft_fee_cap = utils_get_parameter(vault, name="standard_overdraft_fee_cap")
    if not standard_overdraft_fee_cap == 0:
        standard_overdraft_daily_fee_balance = balances[
            "ACCRUED_OVERDRAFT_FEE_RECEIVABLE", DEFAULT_ASSET, denomination, Phase.COMMITTED
        ].net
            ...
            if (
                ...
                posting_instructions.extend(
                    vault.make_internal_transfer_instructions(
                        ...
                        client_transaction_id=f"{INTERNAL_POSTING}_STANDARD_OVERDRAFT_TRANSACTION_FEE_{vault.get_hook_execution_id()}_{denomination}_{posting.client_transaction_id}_{counter}",
                        ...
                        asset=DEFAULT_ASSET,
                        instruction_details={
                            "description": f"Applying standard overdraft transaction fee for {posting.client_transaction_id}",
...
def _autosave_from_purchase(
    ...
    minimum_balance_fee = utils_get_parameter(name="minimum_balance_fee", vault=vault)
    balances = vault.get_balance_timeseries().before(timestamp=effective_date)
    available_balance_before = utils_get_available_balance(balances, denomination)
    proposed_amount = postings.balances()[
        DEFAULT_ADDRESS, DEFAULT_ASSET, denomination, Phase.COMMITTED
    ].net
    ...
    for posting in postings:
        ...
        txn_type = utils_get_transaction_type(
    ...
    if total_savings > 0 and available_balance - total_savings > 0:
        posting_instructions.extend(
            vault.make_internal_transfer_instructions(
                ...
                denomination=denomination,
                client_transaction_id=f"{EXTERNAL_POSTING}_AUTOSAVE_{vault.get_hook_execution_id()}",
                from_account_id=vault.account_id,
                from_account_address=DEFAULT_ADDRESS,
...
def _check_transaction_type_limits(
    ...
    limit
    """
    daily_atm_withdrawal_limit = utils_get_parameter(vault, name="daily_atm_withdrawal_limit")
    txn_code_to_type_map = utils_get_parameter(
        name="transaction_code_to_type_map", is_json=True, vault=vault
    )
    for posting in postings:
        ...
        if txn_type == "ATM withdrawal" and posting.denomination == denomination:
            ...
            if (
                ...
                if (
                    ...
                    raise Rejected(
                        "Transaction would cause the ATM daily withdrawal limit of %s %s to be exceeded."
...
def _accrue_interest(vault, effective_date: datetime) -> list[PostingInstruction]:
    ...
    """
    denomination = utils_get_parameter(name="denomination", vault=vault)
    ...
    if effective_balance > Decimal("0"):
        balance_tier_ranges = utils_get_parameter(vault, "deposit_tier_ranges", is_json=True)
        deposit_interest_rate_tiers = utils_get_parameter(
    ...
    payable_receivable_mapping = vault.modules["interest"].construct_payable_receivable_mapping(
        ...
        payable_internal_account=utils_get_parameter(vault, "accrued_interest_payable_account"),
        paid_internal_account=utils_get_parameter(vault, "interest_paid_account"),
        receivable_internal_account=utils_get_parameter(
            vault, "accrued_interest_receivable_account"
        ),
        received_internal_account=utils_get_parameter(vault, "interest_received_account"),
    )
    accrual_details = vault.modules["interest"].construct_accrual_details(
        ...
        base=utils_get_parameter(name="interest_accrual_days_in_year", vault=vault, union=True),
        net_postings=False,
    )
...
def _apply_monthly_fees(vault, effective_date: datetime) -> list[PostingInstruction]:
    ...
    * Minimum balance fee
    ...
    :return: posting instructions
    """
    maintenance_fee_income_account = utils_get_parameter(
        vault, name="maintenance_fee_income_account"
    )
    inactivity_fee_income_account = utils_get_parameter(vault, name="inactivity_fee_income_account")
    minimum_balance_fee_income_account = utils_get_parameter(
        vault, name="minimum_balance_fee_income_account"
    )
    denomination = utils_get_parameter(vault, name="denomination")
    minimum_balance_fee = utils_get_parameter(vault, name="minimum_balance_fee")
    account_inactivity_fee = utils_get_parameter(vault, name="account_inactivity_fee")
    tier_names = utils_get_parameter(name="tier_names", is_json=True, vault=vault)
    monthly_fee_tiers = _get_monthly_maintenance_fee_tiers(vault)
    minimum_balance_tiers = utils_get_parameter(
        name="minimum_balance_threshold", is_json=True, vault=vault
    )
    minimum_deposit_tiers = utils_get_parameter(
        name="minimum_deposit_threshold", is_json=True, vault=vault
    )
    this_account_flags = utils_get_active_account_flags(vault, tier_names)
    is_account_dormant = vault.get_flag_timeseries(flag=DORMANCY_FLAG).latest()
    monthly_fee = utils_get_dict_value_based_on_account_tier_flag(
        active_flags=this_account_flags,
        tiered_param=monthly_fee_tiers,
        tier_names=tier_names,
        convert=Decimal,
    )
    minimum_balance = utils_get_dict_value_based_on_account_tier_flag(
        active_flags=this_account_flags,
        tiered_param=minimum_balance_tiers,
        tier_names=tier_names,
        convert=Decimal,
    )
    minimum_deposit = utils_get_dict_value_based_on_account_tier_flag(
        active_flags=this_account_flags,
    ...
    if apply_maintenance_fee:
        posting_instructions.extend(
            vault.make_internal_transfer_instructions(
                ...
                client_transaction_id=f"{INTERNAL_POSTING}_APPLY_MONTHLY_FEES_MAINTENANCE_{vault.get_hook_execution_id()}_{denomination}",
                instruction_details={
    ...
    if is_account_dormant and account_inactivity_fee > 0:
        posting_instructions.extend(
            vault.make_internal_transfer_instructions(
                ...
                asset=DEFAULT_ASSET,
                override_all_restrictions=True,
                client_transaction_id=f"{INTERNAL_POSTING}_APPLY_MONTHLY_FEES_INACTIVITY_{vault.get_hook_execution_id()}_{denomination}",
                instruction_details={
        ...
        if monthly_mean_balance < minimum_balance:
            posting_instructions.extend(
                vault.make_internal_transfer_instructions(
                    ...
                    asset=DEFAULT_ASSET,
                    override_all_restrictions=True,
                    client_transaction_id=f"{INTERNAL_POSTING}_APPLY_MONTHLY_FEES_MEAN_BALANCE_{vault.get_hook_execution_id()}_{denomination}",
                    instruction_details={
...
def _accrue_fees(vault, effective_date: datetime) -> list[PostingInstruction]:
    ...
    """
    overdraft_fee_income_account = utils_get_parameter(vault, name="overdraft_fee_income_account")
    overdraft_fee_receivable_account = utils_get_parameter(
        vault, name="overdraft_fee_receivable_account"
    )
    fee_free_overdraft_limit = utils_get_parameter(vault, name="fee_free_overdraft_limit")
    standard_overdraft_daily_fee = utils_get_parameter(vault, name="standard_overdraft_daily_fee")
    standard_overdraft_fee_cap = utils_get_parameter(vault, name="standard_overdraft_fee_cap")
    denomination = utils_get_parameter(vault, name="denomination")
    balances = vault.get_balance_timeseries().latest()
    effective_balance = balances[DEFAULT_ADDRESS, DEFAULT_ASSET, denomination, Phase.COMMITTED].net
...
def _instruct_posting_batch(
    vault, instructions: list[PostingInstruction], effective_date: datetime, event_type: str
) -> None:
    """
...
def _monthly_mean_balance(
    ...
    if period_start < creation_date:
        period_start = creation_date + timedelta(
            hour=effective_date.hour, minute=effective_date.minute, second=effective_date.second
        )
        if period_start < creation_date:
...
def _sum_deposit_transactions(vault, effective_date: datetime) -> Decimal:
    ...
    total_deposit = sum(
        (
            ...
            if INTERNAL_POSTING not in client_txn_id
...
def _get_accrue_interest_schedule(vault) -> utils_EventTuple:
    """
    ...
    """
    return utils_get_daily_schedule(
...
def _get_next_apply_accrued_interest_schedule(
    ...
    """
    interest_application_day = utils_get_parameter(vault, name="interest_application_day")
    apply_accrued_interest_date = utils_get_next_schedule_datetime(
        vault=vault,
    ...
    apply_accrued_interest_schedule = utils_create_schedule_dict_from_datetime(
...
def _get_next_fee_schedule(vault, effective_date: datetime, period: timedelta) -> dict[str, str]:
    ...
    """
    fees_application_day = utils_get_parameter(vault, name="fees_application_day")
    fees_application_hour = utils_get_parameter(vault, name="fees_application_hour")
    fees_application_minute = utils_get_parameter(vault, name="fees_application_minute")
    fees_application_second = utils_get_parameter(vault, name="fees_application_second")
    creation_date = vault.get_account_creation_date()
    ...
    return utils_create_schedule_dict_from_datetime(next_schedule_date)
...
def _get_overdraft_per_transaction_fee_charged(vault, denomination: str) -> Decimal:
    ...
    daily_fee_charged = sum(
        (
            ...
            if posting.value_timestamp > start_of_period
...
def _get_monthly_maintenance_fee_tiers(vault) -> Any:
    ...
    return utils_get_parameter(name=fee_to_use, is_json=True, vault=vault)
...
def _has_transaction_type_not_covered_by_standard_overdraft(
    ...
    :return: Boolean
    """
    txn_code_to_type_map = utils_get_parameter(vault, "transaction_code_to_type_map", is_json=True)
    optional_standard_overdraft_coverage = utils_get_parameter(
    ...
    return not should_include_optional_standard_overdraft_coverage and any(
        (
            ...
            for posting in postings
            if utils_get_available_balance(posting.balances(), posting.denomination) < 0
        )
    )

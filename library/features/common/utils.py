...
PostingInstructionTypeAlias = Union[AuthorisationAdjustment , CustomInstruction , InboundAuthorisation , InboundHardSettlement , OutboundAuthorisation , OutboundHardSettlement , Release , Settlement , Transfer]
...
END_OF_TIME = datetime(2099, 1, 1, 0, 0, 0, 0, tzinfo=ZoneInfo("Asia/Jakarta"))
...
def validate_amount_precision(amount: Decimal, max_precision: int = 2) -> Rejection:
    """
...
def get_schedule_time_from_parameters(
    vault: SmartContractVault, parameter_prefix: str, daily_schedule: bool=False
) -> tuple[int, int, int, int]:
    if not daily_schedule:
        day = int(get_parameter(vault=vault, name=f"{parameter_prefix}_day"))
    else:
        day = None
...
def get_schedule_expression_from_parameters(
    ...
    *,
    ...
    day, hour, minute, second = get_schedule_time_from_parameters(vault, parameter_prefix, daily_schedule)
    schedule_datetime=effective_datetime.replace(hour=hour, minute=minute,second=second, tzinfo=ZoneInfo(events_timezone))
    schedule_datetime=schedule_datetime.astimezone(tz=ZoneInfo("UTC"))
    return ScheduleExpression(
        hour=str(schedule_datetime.hour),
        minute=str(schedule_datetime.minute),
        second=str(schedule_datetime.second),
        day=None if not day else str(schedule_datetime.day),
        month=None if not month else str(schedule_datetime.month),
        year=None if not year else str(schedule_datetime.year),
...
def create_end_of_time_schedule(
    start_datetime: datetime,
    events_timezone: str
) -> ScheduledEvent:
    """
    ...
    """
    return ScheduledEvent(
        start_datetime=start_datetime.astimezone(tz=ZoneInfo(events_timezone)),
        skip=True,
        expression=END_OF_TIME_EXPRESSION,
    )

def create_end_of_time_schedule_due_amount_calc(vault: SmartContractVault, start_datetime: datetime, events_timezone:str) -> ScheduledEvent:
    """
    Sets up a dummy schedule with the End of Time expression, that will never produce any
    jobs and is skipped indefinitely.
    ...
    :return: returns a dummy scheduled event
    """
    start_datetime = start_datetime.astimezone(tz=ZoneInfo(events_timezone)) - relativedelta(seconds=1)
    account_creation_datetime = vault.get_account_creation_datetime().astimezone(tz=ZoneInfo(events_timezone))
    if start_datetime < account_creation_datetime:
        start_datetime = account_creation_datetime
    return ScheduledEvent(
        start_datetime=start_datetime.astimezone(tz=ZoneInfo(events_timezone)),

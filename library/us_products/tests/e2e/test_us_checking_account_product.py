...
from inception_sdk.vault.postings.postings_helper import (
    create_pib_from_posting_instructions,
)
...
endtoend.testhandle.WORKFLOWS = {
    "US_CHECKING_ACCOUNT_APPLICATION": (
        "library/us_products/workflows/us_checking_account_application.yaml"
    ),
    "US_PRODUCTS_CLOSURE": "library/us_products/workflows/us_products_closure.yaml",
    "US_PRODUCTS_MANAGE_OVERDRAFT_COVERAGE": (
        "library/us_products/workflows/us_products_manage_overdraft_coverage.yaml"
    ),
}
...
class CheckingAccountProductTest(endtoend.End2Endtest):
    ...
    def test_workflow_apply_for_us_checking_account_with_overdraft(self):
        ...
        self.assertEqual(
            ...
            account_schedules["APPLY_ACCRUED_OVERDRAFT_INTEREST"]["next_run_timestamp"],
        )
        self.assertEqual(
            apply_interest_expected_next_run_time.strftime("%Y-%m-%dT%H:%M:%SZ"),

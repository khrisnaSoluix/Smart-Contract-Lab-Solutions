...
event_types = [
    ...
    EventType(
        ...
        scheduler_tag_ids=["US_SAVINGS_APPLY_ACCRUED_INTEREST_AST"],
    ),
    EventType(
        name="APPLY_MONTHLY_FEES",
        scheduler_tag_ids=["US_SAVINGS_APPLY_MONTHLY_FEES_AST"],
    ),
    EventType(name="APPLY_ANNUAL_FEES", scheduler_tag_ids=["US_SAVINGS_APPLY_ANNUAL_FEES_AST"]),
]
...
parameters = [
    ...
    Parameter(
        ...
        default_value=json_dumps(
            {
                "US_SAVINGS_ACCOUNT_TIER_UPPER": {"tier1": "0.02"},
                "US_SAVINGS_ACCOUNT_TIER_MIDDLE": {"tier1": "0.0125"},
                "US_SAVINGS_ACCOUNT_TIER_LOWER": {"tier1": "0.01"},
            }
        ),
    ),
    Parameter(
    ...
    Parameter(
        ...
        level=Level.TEMPLATE,
        description="The minimum combined daily checking and savings account balance required for"
        ...
        " balances at the fee application time. This check is only done when an account"
        " is under a supervisor plan, and this parameter is otherwise unused.",
    ...
    Parameter(
        ...
        " fee can be waived if one automated deposit transfer is made for the period.",
    ...
    Parameter(
        ...
        level=Level.TEMPLATE,
        description="The promotional monthly fee charged "
        "for account maintenance for each account tier.",
    ...
    Parameter(
        ...
        shape=StringShape,
        level=Level.TEMPLATE,
        description="The tag to check for in the client transaction id that identifies a"
        " qualifying automated transfer. Used in determining waive criteria for monthly"
        " maintenance fee.",
    ...
    Parameter(
        ...
        shape=NumberShape(min_value=0, max_value=23, step=1),
        level=Level.TEMPLATE,
        description="The hour of the day at which maintenance and minimum balance fees"
        " are applied",
    ...
    Parameter(
        ...
        shape=NumberShape(min_value=0, max_value=59, step=1),
        level=Level.TEMPLATE,
        description="The minute of the hour at which maintenance and minimum balance fees"
        " are applied",
    ...
    Parameter(
        ...
        shape=NumberShape(min_value=0, max_value=59, step=1),
        level=Level.TEMPLATE,
        description="The second of the minute at which maintenance and minimum balance fees"
        " are applied.",
    ...
    Parameter(
        ...
        level=Level.TEMPLATE,
        description="The minimum balance allowed for each account tier. If the mean"
        " daily balance falls below this, the fee will be charged. The"
        " calculation is performed every month on the anniversary of the account"
        " opening day, for each day since the last calculation. It takes samples of"
        " the balance at the fee application time.",
...
def execution_schedules():
    ...
    apply_annual_fees_schedule = _get_next_apply_fees_schedule(
...
def scheduled_code(event_type, effective_date):
    if event_type == "ACCRUE_INTEREST":
        ...
        _apply_fees(vault, effective_date, ["maintenance_fee_annual"])
        vault.amend_schedule(
            event_type="APPLY_ANNUAL_FEES",
            new_schedule=_get_next_apply_fees_schedule(vault, effective_date, timedelta(years=1)),
        )
...
def pre_posting_code(postings, effective_date):
    ...
    # TODO: Once supervisors support the PostingInstruction and PostingInstructionBatch balances()
    # methods we must use these instead of amount and credit attributes
    for post in postings:
        if post.amount < min_withdrawal and not post.credit:
            raise Rejected(
        ...
        if post.amount < min_deposit and post.credit:
            raise Rejected(
    ...
    proposed_amount = _sum_of_postings_amount(postings)
    ...
    # Check total proposed incoming not greater than max balance
...
def _reschedule_apply_accrued_interest_event(vault, effective_date):
    ...
    vault.amend_schedule(event_type="APPLY_ACCRUED_INTEREST", new_schedule=apply_interest_schedule)
...
def _reverse_accrued_interest(vault, effective_date):
    """
...
def _apply_fees(vault, effective_date, fee_parameters):
    """
    ...
    :param fee_parameters: list<string>, names of fee template parameters that can
                           be applied ('maintenance_fee_monthly', 'maintenance_fee_annual',
                           'minimum_balance_fee')
    """
    ...
    for fee_parameter in fee_parameters:
        ...
        # Post fee amount if set for this account
        if fee_amount > 0:
            posting_instructions.extend(
                vault.make_internal_transfer_instructions(
                    ...
                    client_transaction_id=f"{INTERNAL_POSTING}_APPLY_{fee_parameter.upper()}"
...
def _get_monthly_mean_balance(vault, denomination, effective_date):
    """
    ...
    for posting in postings:
        ...
        if not posting.credit:
            ...
            if (
                ...
                raise Rejected(
                    "Transaction would cause the maximum"
            ...
            if abs(amount_deposit + amount_authed) > max_daily_deposit and max_daily_deposit != 0:
                raise Rejected(
                    "Transaction would cause the maximum"
                    " daily deposit limit of %s %s to be exceeded."
...
def _get_latest_available_balance(
    balances, denomination, excluded_phase, address=DEFAULT_ADDRESS, asset=DEFAULT_ASSET
):
    """
...
def _get_next_interest_application_day(vault, effective_date):
    """
...
def _get_next_apply_accrued_interest_schedule(vault, effective_date):
    """
...
def _get_next_apply_interest_datetime(vault, effective_date):
    """
...
def _get_next_apply_fees_schedule(vault, effective_date, period):
    """
...
def _sum_of_postings_amount(postings):
    """
    Net of all postings in a batch for DEFAULT_ADDRESS.
...
def _sum_without_current_client_trans(
    denomination, client_transactions, client_transaction_id, cutoff_timestamp
):
    """
...
def _count_automated_deposit_transactions(vault, effective_date) -> int:
    """
...
def _get_tiered_interest_rates(vault):
    """
...
def _handle_excess_withdrawals(
    ...
    if (
        ...
        vault.start_workflow(
            workflow="US_PRODUCTS_TRANSACTION_LIMIT_WARNING",
            context={

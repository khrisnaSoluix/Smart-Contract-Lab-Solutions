...
#    credit_card.py
# md5:16b259680bf5da3a65eca4f8f9aee6ac
...
from typing import Callable, Optional, Union, Any, Iterable, Mapping
...
def activation_hook(
    ...
    payment_due_period = int(utils_get_parameter(vault, name=PARAM_PAYMENT_DUE_PERIOD))
    ...
    scheduled_events[EVENT_ACCRUE] = ScheduledEvent(
        ...
        expression=utils_get_schedule_expression_from_parameters(vault, ACCRUAL_SCHEDULE_PREFIX),
    )
    ...
    scheduled_events[EVENT_ANNUAL_FEE] = ScheduledEvent(
        start_datetime=effective_datetime,
        expression=utils_get_schedule_expression_from_parameters(vault, ANNUAL_FEE_SCHEDULE_PREFIX),
    )
    ...
    credit_limit = utils_get_parameter(name=PARAM_CREDIT_LIMIT, vault=vault)
    denomination = utils_get_parameter(name=PARAM_DENOMINATION, vault=vault)
...
def deactivation_hook(
    ...
    denomination: str = utils_get_parameter(name=PARAM_DENOMINATION, vault=vault)
    account_closure_flags_applied = utils_is_flag_in_list_applied(
        vault=vault,
        parameter_name=PARAM_ACCOUNT_CLOSURE_FLAGS,
        effective_datetime=effective_datetime,
    )
    write_off_flags_applied = utils_is_flag_in_list_applied(
        vault=vault,
        parameter_name=PARAM_ACCOUNT_WRITE_OFF_FLAGS,
        effective_datetime=effective_datetime,
    )
    if rejection := _can_final_statement_be_generated(
        in_flight_balances, account_closure_flags_applied, write_off_flags_applied, denomination
    ):
...
def pre_posting_hook(
    ...
    txn_code_to_type_map = utils_get_parameter(
    ...
    if txn_type_validation_rejection:
        return PrePostingHookResult(rejection=txn_type_validation_rejection)
    ...
    if insufficient_fund_rejections:
        return PrePostingHookResult(rejection=insufficient_fund_rejections)
    ...
    if txn_type_credit_limit_rejections:
        return PrePostingHookResult(rejection=txn_type_credit_limit_rejections)
    ...
    if txn_type_time_limit_rejections:
        return PrePostingHookResult(rejection=txn_type_time_limit_rejections)
    return None
...
@fetch_account_data(event_type="ACCRUE_INTEREST", balances=["one_second_to_midnight_bif"])
@requires(
...
@fetch_account_data(event_type="STATEMENT_CUT_OFF", balances=["statement_cutoff_to_live_bif"])
@requires(event_type="ANNUAL_FEE", parameters=True)
@fetch_account_data(event_type="ANNUAL_FEE", balances=["live_balances_bof"])
@requires(event_type="PAYMENT_DUE", parameters=True, flags=True)
@fetch_account_data(event_type="PAYMENT_DUE", balances=["live_balances_bof"])
def scheduled_event_hook(
    ...
    if event_type == EVENT_ACCRUE:
        ...
        annual_fee_schedule_expression = utils_get_schedule_expression_from_parameters(
            vault,
        ...
        (local_next_pdd_start, local_next_pdd_end) = _get_next_pdd(
            payment_due_period, account_creation_dt, last_pdd_execution_datetime=effective_datetime
        )
        (_, local_next_scod_end) = _get_scod_for_pdd(payment_due_period, local_next_pdd_start)
        scod_schedule_expression = utils_get_schedule_expression_from_parameters(
            vault,
...
ParameterTypes = Union[datetime, Decimal, int, OptionalValue, str, UnionItemValue]
BILLED = "BILLED"
CHARGED = "CHARGED"
AUTH = "AUTH"
UNPAID = "UNPAID"
...
CHARGED_BALANCE_STATES = [BILLED, CHARGED, UNPAID]
...
address_details = [
    ...
    AddressDetails(
        account_address=DEPOSIT_BALANCE, description="Tracks repayments into the account", tags=[]
    ),
    AddressDetails(
    ...
    AddressDetails(
        ...
        description="Used for double-entry bookkeeping when making postings to adjust other balances (unless there is a defined internal account for the posting).",
        tags=[],
    ),
    AddressDetails(
        account_address=MAD_BALANCE,
        description="Minimum amount that the customer must repay before the end of the payment due date (incl grace) to avoid becoming delinquent",
        tags=[],
    ),
    AddressDetails(
        account_address=OUTSTANDING_BALANCE,
        description="The sum of all outstanding settled transactions, charged/billed fees and billed interest",
        tags=[],
    ),
    AddressDetails(
        account_address=REVOLVER_BALANCE,
        description="Defines whether the account is revolver or not. Set to 1 if revolver and 0 if transactor.",
        tags=[],
    ),
    AddressDetails(
        account_address=STATEMENT_BALANCE,
        description="Amount billed to the client for the statement. Note this is equivalent to outstanding/full outstanding at the time of statement processing, as all charged transactions and bank charges will have been billed.",
        tags=[],
    ),
    AddressDetails(
        account_address=TRACK_STATEMENT_REPAYMENTS,
        description="Sum of all repayments that were made during a statement cycle. This is reset  to 0 at the end of each SCOD",
        tags=[],
    ),
]
...
FEE_TYPE = "fee_type"
LIVE_BALANCES_BOF_ID = fetchers_LIVE_BALANCES_BOF_ID
...
parameters = [
    Parameter(
        ...
        description="Additional limit on top of credit limit available to spend. Might involve fees.",
    ...
    Parameter(
        ...
        level=ParameterLevel.INSTANCE,
        description='Indicates whether the customer has opted in to the overlimit facility. If "True" the customer can exceed the credit limit by the overlimit for regular transactions and stand-in/offline transactions. Otherwise, the customer can only exceed the credit limit for stand-in/offline transactions.',
    ...
    Parameter(
        name=PARAM_TXN_REFS,
        level=ParameterLevel.INSTANCE,
        description="Map of lists of Transaction types and their associated references (map format - encoded json).",
    ...
    Parameter(
        ...
        level=ParameterLevel.INSTANCE,
        description='Map of limits per transaction type (map format - encoded json). A transaction must respect overall limits and transaction type limits. For credit limit checks the sum of authorised and outstanding amounts for a given transaction type can be subject to an absolute limit, using the "flat" key, and/or a relative limit with respect to the credit limit, using the "percentage" key. If both are specified, the lowest of the two is applied. If either is missing it is assumed to not apply. If a transaction type has no entry, no specific limits apply. "allowed_days_after_opening" is a time-based check that permits transactions only in a window after the account is activated.',
    ...
    Parameter(
        ...
        level=ParameterLevel.INSTANCE,
        description='Map of map of fees per transaction type (map format - encoded json). Allows a "over_deposit_only", "percentage_fee" and "flat_fee" to be specifiedfor transactions of the given type. The highest fee will be selected based on the transaction amount and charged on the next statement. If a transaction type does not have an entry, no fees apply. If "over_deposit_only" is set to True, the associated type will only charge a fee if the transaction amount exceeds the deposit balance',
    ...
    Parameter(
        name=PARAM_TXN_APR,
        level=ParameterLevel.INSTANCE,
        description="Map of maps of Annual Percentage Rate per transaction ref (map format - encoded json)",
    ...
    Parameter(
        ...
        level=ParameterLevel.INSTANCE,
        description="Map of maps of Per annum gross interest rate per transaction ref (map format - encoded json)",
    ...
    Parameter(
        ...
        level=ParameterLevel.INSTANCE,
        description="Number of days after SCOD that payment is due by. Minimum Amount Due must be paid back by then to avoid Late Repayment fees. Full outstanding balance must be paid back by then to avoid becoming revolver.",
        display_name="Payment Due Period",
        shape=NumberShape(min_value=21, max_value=27, step=1),
    ...
    Parameter(
        ...
        description="List of interest free period expiry times associated with transaction types.This is for transaction types that do not use transaction references",
    ...
    Parameter(
        ...
        description="List of interest free period expiry times associated with transaction references.",
    ...
    Parameter(
        ...
        shape=StringShape(),
        default_value=dumps(
            {"xxx": "purchase", "aaa": "cash_advance", "cc": "transfer", "bb": "balance_transfer"}
        ),
    ),
    Parameter(
        name=PARAM_TXN_TYPES,
        level=ParameterLevel.TEMPLATE,
        description="Map of maps of supported transaction types for the account, with any non-default parameters specified. All default to False. (map format - encoded json).",
    ...
    Parameter(
        ...
        description="Map of transaction type to internal account id for transaction type fee purposes (map format - encoded json).",
        level=ParameterLevel.TEMPLATE,
        shape=StringShape(),
        default_value=dumps(
            {
    ...
    Parameter(
        ...
        level=ParameterLevel.TEMPLATE,
        description="Map of transaction type to internal account id for interest purposes. ContainsInterest Income account for each transaction type (map format - encoded json).",
    ...
    Parameter(
        ...
        level=ParameterLevel.TEMPLATE,
        description="Per annum gross interest rate per transaction type (map format - encoded json)",
        display_name="Per Annum Gross Interest Rate Per Transaction Type",
        shape=StringShape(),
        default_value=dumps(
            {"purchase": "0.01", "cash_advance": "0.02", "transfer": "0.03", "fees": "0.01"}
        ),
    ),
    Parameter(
        name=PARAM_ACCRUE_INTEREST_ON_UNPAID_INTEREST,
        description='Interest accrual on unpaid interest. If set to "False", interest will not be calculated and applied on unpaid interest.',
    ...
    Parameter(
        ...
        description='Interest accrual on unpaid fees. If set to "False", interest will not be accrued on unpaid fees.',
    ...
    Parameter(
        ...
        level=ParameterLevel.TEMPLATE,
        description="For transactions that are not affected by specific interest behaviours (i.e. an active Interest Free Period or transaction types that always charge interest from the transaction date), determines the start point for interest accrual on transactions charged when entering Revolver status. For more information please refer to the product documentation.",
    ...
    Parameter(
        ...
        description="Internal account used to write-off any outstanding fees and transactions. The name follows the accounting definition of Principal",
    ...
    Parameter(
        ...
        shape=StringShape(),
        default_value=dumps({"purchase": "0.01", "cash_advance": "0.02", "transfer": "0.03"}),
    ),
    Parameter(
        name=PARAM_MAD,
        level=ParameterLevel.TEMPLATE,
        description="Minimum amount repayment due (higher value of minimum_amount_due or minimum_percentage_due used)",
    ...
    Parameter(
        ...
        level=ParameterLevel.TEMPLATE,
        description="Percentage of statement balance per transaction type used to calculate the minimum amount due (higher value of minimum_amount_due or minimum_percentage_due is used)",
        display_name="Percentage Of Statement Balance Per Transaction Type (map format - encoded json) ",
        default_value=dumps(
            {
                "purchase": "0.01",
                "cash_advance": "0.01",
    ...
    Parameter(
        ...
        description="External fees that can be initiated from outside the contract, but need to be stored in separate addresses. Stored as an encoded json list.",
    ...
    Parameter(
        ...
        description='Income internal account for each external fee type. Stored as an encoded json map of fee type to json map of account type to account id. The account type is "income".',
        display_name="Credit Card Dispute Fee Account",
        level=ParameterLevel.TEMPLATE,
        shape=StringShape(),
        default_value=dumps({"dispute_fee": "FEE_INCOME", "withdrawal_fee": "FEE_INCOME"}),
    ),
    Parameter(
    ...
    Parameter(
        ...
        description="List of flags applied to customer or account when account closure has beenrequested or imposed (i.e. customer or bank initiated)",
    ...
    Parameter(
        ...
        description="List of flags applied to customer or account which, when applied will generatepostings to zero out FULL_OUTSTANDING_BALANCE on request of account closure",
    ...
    Parameter(
        ...
        description="List of flags applied to customer or account which, when applied will suspend the internal address transfers from billed to unpaid balances on PDD",
    ...
    Parameter(
        ...
        description="List of flags applied to customer or account which, when applied will set PARAM_MAD equal to statement balance",
    ...
    Parameter(
        ...
        description='List of flags applied to customer or account which, when applied will set PARAM_MAD to zero by the next SCOD/PDD event. In effect, the customer will not have to pay any PARAM_MAD for the statement periods where this is active, and no late repayment fees will be chargedNote that this takes precedence over the "PARAM_MAD as Full Statement Flags" parameter.',
    ...
    Parameter(
        ...
        description="List of flags applied to customer or account which, when applied will suspend the internal address updates on PDD that age the overdue balance buckets",
    ...
    Parameter(
        ...
        description="The hour at which the ACCRUE_INTEREST schedule should execute for all CC accounts.",
    ...
    Parameter(
        ...
        description="The minute at which the ACCRUE_INTEREST schedule should execute for all CC accounts.",
    ...
    Parameter(
        ...
        description="The second at which the ACCRUE_INTEREST schedule should execute for all CC accounts.",
    ...
    Parameter(
        ...
        description="The hour at which the STATEMENT_CUT_OFF schedule should execute for all CC accounts.",
    ...
    Parameter(
        ...
        description="The minute at which the STATEMENT_CUT_OFF schedule should execute for all CC accounts.",
    ...
    Parameter(
        ...
        description="The second at which the STATEMENT_CUT_OFF schedule should execute for all CC accounts.",
    ...
    Parameter(
        ...
        description="The hour at which the PAYMENT_DUE schedule should execute for all CC accounts.",
    ...
    Parameter(
        ...
        description="The minute at which the PAYMENT_DUE schedule should execute for all CC accounts.",
    ...
    Parameter(
        ...
        description="The second at which the PAYMENT_DUE schedule should execute for all CC accounts.",
    ...
    Parameter(
        ...
        description="The hour at which the ANNUAL_FEE schedule should execute for all CC accounts.",
    ...
    Parameter(
        ...
        description="The minute at which the ANNUAL_FEE schedule should execute for all CC accounts.",
    ...
    Parameter(
        ...
        description="The second at which the ANNUAL_FEE schedule should execute for all CC accounts.",
...
def _zero_out_balances_for_account_closure(
    ...
    if accrue_interest_from_txn_day:
        accrued_interest_instructions += _reverse_uncharged_interest(
            vault, in_flight_balances, denomination, txn_types, "ACCOUNT_CLOSED", PRE_SCOD
        )
        accrued_interest_instructions += _reverse_uncharged_interest(
            vault, in_flight_balances, denomination, txn_types, "ACCOUNT_CLOSED", POST_SCOD
        )
...
def _process_write_off(
    ...
    principal_write_off_account = utils_get_parameter(
        vault, name=PARAM_PRINCIPAL_WRITE_OFF_INTERNAL_ACCOUNT
    )
    interest_write_off_account = utils_get_parameter(
        vault, name=PARAM_INTEREST_WRITE_OFF_INTERNAL_ACCOUNT
    )
    credit_limit = utils_get_parameter(
    ...
    adjustment_posting_instructions = _adjust_aggregate_balances(
        vault, denomination, in_flight_balances, effective_datetime, credit_limit=credit_limit
    )
...
def _can_final_statement_be_generated(
    ...
    if not are_write_off_flags_applied:
        full_outstanding_balance = utils_balance_at_coordinates(
    ...
    for posting_instruction in posting_instructions:
        ...
        if not txn_ref:
            if txn_type.upper() in txn_types_with_refs:
                return Rejection(
                    message=f"Transaction type {txn_type} requires a transaction level reference and none has been specified.",
            ...
            return Rejection(
                message=f"{txn_ref} undefined in parameters for {txn_type}. Please update parameters.",
                reason_code=RejectionReason.AGAINST_TNC,
            )
        for dimensions in balances.keys():
            if dimensions[0] == _principal_address(txn_type, CHARGED, txn_ref=txn_ref):
                return Rejection(
                    message=f"{txn_ref} already in use for {txn_type}. Please select a unique reference.",
                    reason_code=RejectionReason.AGAINST_TNC,
                )
    return None
...
def _check_account_has_sufficient_funds(
    ...
    if overlimit_amount > 0:
        return Rejection(
            message=f"Insufficient funds for {denomination} {-available_balance_delta} transaction. Overlimit already in use",
            reason_code=RejectionReason.INSUFFICIENT_FUNDS,
        )
    opt_in = utils_get_parameter(
        vault, name=PARAM_OVERLIMIT_OPT_IN, is_optional=True, is_boolean=True, default_value="False"
    )
    if opt_in:
        overlimit = utils_get_parameter(
    ...
    if available_balance + available_balance_delta < 0:
        return Rejection(
            message=f"Insufficient funds {denomination} {available_balance} for {denomination} {-available_balance_delta} transaction (excl advice instructions)",
            reason_code=RejectionReason.INSUFFICIENT_FUNDS,
        )
    return None
...
def _check_txn_type_time_limits(
    vault: Any, posting_instructions: list[PostingInstruction], effective_datetime: datetime
) -> Optional[Rejection]:
    """
    ...
    """
    txn_type_limits: dict[str, dict[str, str]] = utils_get_parameter(
    ...
    for instruction in posting_instructions:
        ...
        if allowed_days_after is not None:
            ...
            if effective_datetime >= end_of_allowed_period:
                return Rejection(
                    message=f"Transaction not permitted outside of configured window {allowed_days_after} days from account opening",
                    reason_code=RejectionReason.AGAINST_TNC,
                )
    return None
...
def _handle_credit_limit_change(
    ...
    """
    if not utils_has_parameter_value_changed(
        parameter_name=PARAM_CREDIT_LIMIT,
        old_parameters=old_parameter_values,
        updated_parameters=updated_parameter_values,
    ):
        return []
    new_credit_limit: Decimal = updated_parameter_values[PARAM_CREDIT_LIMIT]
    old_credit_limit: Decimal = old_parameter_values[PARAM_CREDIT_LIMIT]
    denomination = utils_get_parameter(name=PARAM_DENOMINATION, vault=vault)
...
def _rebalance_postings(
    ...
    for posting_instruction in posting_instructions:
        ...
        client_transaction = client_transactions[posting_instruction.unique_client_transaction_id]
        if posting_instruction.type in (
...
def _charge_txn_type_fees(
    ...
    txn_type_fees = utils_get_parameter(name=PARAM_TXN_TYPE_FEES, is_json=True, vault=vault)
    deposit_balance_before_txn = utils_balance_at_coordinates(
    ...
    for posting_instruction in posting_instructions:
        ...
        combine_flat_and_percentage = utils_str_to_bool(fees.get("combine", "False"))
        over_deposit_only = utils_str_to_bool(fees.get("over_deposit_only", "False"))
        if not over_deposit_only or deposit_balance_before_txn < posting_instruction_balance:
            if combine_flat_and_percentage:
                fee_amount = sum([flat_fee, percentage_fee * posting_instruction_balance])
            else:
...
def _check_txn_type_credit_limits(
    ...
    """
    txn_type_credit_limits = utils_get_parameter(
        name=PARAM_TXN_TYPE_LIMITS, at_datetime=effective_datetime, is_json=True, vault=vault
    )
    ...
    txn_types_with_refs: dict[str, list[str]] = utils_get_parameter(
        vault, name=PARAM_TXN_REFS, at_datetime=effective_datetime, is_json=True
    )
    txn_types_with_refs = {
    ...
    for (txn_type, proposed_amount) in proposed_amount_by_txn_type.items():
        ...
        if proposed_amount + current_txn_type_balance > final_txn_type_credit_limits[txn_type]:
            return Rejection(
                message=f"Insufficient funds for {denomination} {abs(proposed_amount)} transaction due to {denomination} {final_txn_type_credit_limits[txn_type]:.2f} limit on transaction type {txn_type}. Outstanding transactions amount to {denomination} {abs(current_txn_type_balance)}",
                reason_code=RejectionReason.INSUFFICIENT_FUNDS,
            )
    return None
...
def _repay_spend_and_charges(
    ...
    txn_repayment_hierarchy: dict[str, dict[str, str]] = utils_get_parameter(
        vault, name=PARAM_TXN_APR, is_json=True
    )
    txn_repayment_hierarchy = {
        key: {str(i).upper(): str(j).upper() for (i, j) in value.items()}
        for (key, value) in txn_repayment_hierarchy.items()
    }
    txn_type_repayment_hierarchy = utils_get_parameter(vault, name=PARAM_APR, is_json=True)
    ...
    for (_, _, address) in repayment_addresses:
        if remaining_repayment_amount > 0:
            balance = utils_balance_at_coordinates(
...
def _create_custom_instructions(
    ...
    if amount == 0:
        return []
    denomination = denomination or utils_get_parameter(vault, name=PARAM_DENOMINATION)
...
def _adjust_aggregate_balances(
    ...
    if (
        utils_balance_at_coordinates(
            balances=in_flight_balances, denomination=denomination, address=FULL_OUTSTANDING_BALANCE
        )
        <= 0
        and scope[FULL_OUTSTANDING_BALANCE]
    ):
        adjustment_posting_instructions.extend(
            _change_revolver_status(vault, denomination, in_flight_balances, revolver=False)
        )
    return adjustment_posting_instructions


def _change_revolver_status(
    vault: Any, denomination: str, in_flight_balances: BalanceDefaultDict, revolver: bool
) -> list[CustomInstruction]:
    """
...
def _rebalance_outbound_settlement(
    ...
    external_fee_types = utils_get_parameter(vault, name=PARAM_EXTERNAL_FEE_TYPES, is_json=True)
    newest_posting_instruction = client_transaction.posting_instructions[-1]
    client_transaction_denomination: str = client_transaction.denomination
    ...
    (credit_line_spend, deposit_spend) = _determine_amount_breakdown(
        amount_to_settle, client_transaction_denomination, in_flight_balances
    )
...
def _get_interest_internal_accounts(vault: Any, charge_type: str, sub_type: str) -> str:
    """
    ...
    """
    if charge_type == FEES:
        income_account: str = utils_get_parameter(
            name=PARAM_INTEREST_ON_FEES_INTERNAL_ACCOUNT, vault=vault
        )
    else:
        txn_type_interest_internal_account_map: dict[str, str] = utils_get_parameter(
...
def _determine_amount_breakdown(
    amount_to_charge: Decimal, denomination: str, in_flight_balances: BalanceDefaultDict
    ...
    deposit
    """
    available_deposit = utils_balance_at_coordinates(
...
def _update_auth_bucket_for_outbound_settlement(
    ...
    prior_unsettled_amount = -client_transaction.effects(
        effective_datetime=client_transaction.posting_instructions[-2].value_datetime
    ).unsettled
    settled_amount = _get_settlement_amount(
        account_id=vault.account_id,
        denomination=client_transaction.denomination,
        posting_instruction=client_transaction.posting_instructions[-1],
    )
    if client_transaction.completed() or settled_amount >= prior_unsettled_amount:
        auth_bucket_delta = prior_unsettled_amount
    else:
        auth_bucket_delta = settled_amount
...
def _get_settlement_amount(
    account_id: str, denomination: str, posting_instruction: PostingInstruction
) -> Decimal:
    """
...
def _get_settlement_info(
    ...
    """
    unsettled_amount = (
        _get_unsettled_amount(denomination, client_transaction)
...
def _get_unsettled_amount(denomination: str, client_transaction: ClientTransaction) -> Decimal:
    """
...
def _is_revolver(balances: BalanceDefaultDict, denomination: str) -> bool:
    ...
    """
    revolver_balance = balances[REVOLVER_BALANCE, DEFAULT_ASSET, denomination, Phase.COMMITTED].net
...
def _rebalance_interest(
    ...
    (credit_line_amount, deposit_amount) = _determine_amount_breakdown(
        amount_to_charge=amount, denomination=denomination, in_flight_balances=in_flight_balances
    )
...
def _rebalance_fees(
    ...
    """
    amount = utils_round_decimal(amount, 2)
    if amount == 0:
        return []
    rebalance_fees_posting_instructions: list[CustomInstruction] = []
    external_fee_types = [
        fee.upper()
        for fee in utils_get_parameter(vault, name=PARAM_EXTERNAL_FEE_TYPES, is_json=True)
    ]
    (credit_line_amount, deposit_amount) = _determine_amount_breakdown(
        amount_to_charge=amount, denomination=denomination, in_flight_balances=in_flight_balances
    )
    ...
    rebalance_fees_posting_instructions.extend(
        _make_deposit_postings(
            vault, denomination, deposit_amount, in_flight_balances, instruction_details
        )
    )
...
def _process_interest_accrual_and_charging(
    ...
    """
    if utils_is_flag_in_list_applied(
        vault=vault,
    ...
    txn_types_with_params: dict[str, dict[str, str]] = utils_get_parameter(
...
def _accrue_interest(
    ...
    """
    accrue_interest_on_unpaid_interest = utils_get_parameter(
        vault,
        name=PARAM_ACCRUE_INTEREST_ON_UNPAID_INTEREST,
        is_optional=True,
        is_boolean=True,
    ...
    accrue_interest_on_unpaid_fees = utils_get_parameter(
        vault,
        name=PARAM_ACCRUE_INTEREST_ON_UNPAID_FEES,
        is_optional=True,
        is_boolean=True,
    ...
    accrue_interest_from_txn_day = utils_get_parameter(
    ...
    if not balances_to_accrue_on:
        return {}
    base_interest_rates: dict[str, str] = utils_get_parameter(
        vault, name=PARAM_BASE_INTEREST_RATES, at_datetime=accrual_cut_off_dt, is_json=True
    )
    txn_base_interest_rates: dict[str, dict[str, str]] = utils_get_parameter(
        vault, name=PARAM_TXN_BASE_INTEREST_RATES, at_datetime=accrual_cut_off_dt, is_json=True
    )
    ...
    account_creation_dt = vault.get_account_creation_datetime()
    payment_due_period = int(
        utils_get_parameter(vault, name=PARAM_PAYMENT_DUE_PERIOD, at_datetime=accrual_cut_off_dt)
    )
...
def _get_balances_to_accrue_on(
    ...
    def _update_balances_to_accrue_on(
        ...
        if (
            ...
            and (
                not _is_txn_type_in_interest_free_period(
    ...
    for (txn_type, refs) in supported_txn_types.items():
        for ref in refs or [""]:
            ...
            accrue_interest_on_txn_type_from_txn_day = accrue_interest_from_txn_day and (
                not _is_txn_type_in_interest_free_period(
                    txn_types_in_interest_free_period, txn_type, ref
                )
            )
...
def _calculate_accruals_and_create_instructions(
    ...
    def _create_accrual_instructions(
        ...
        accrual_amount = utils_round_decimal(daily_rate * amount, decimal_places=2)
        if accrual_amount > 0:
            ...
            if (
                ...
                instruction_details = {
                    "description": f"Daily interest accrued at {daily_rate_percent:.7f}%% on balance of {abs(amount):.2f}, for transaction type {stem_with_accrual_type}"
                }
                instructions.extend(
...
def yearly_to_daily_rate(yearly_rate: Decimal, leap_year: bool) -> Decimal:
    ...
    return utils_round_decimal(yearly_rate / days_in_year, decimal_places=10)
...
def _get_previous_scod(
    vault: Any, account_creation_datetime: datetime
...
def _get_next_pdd(
    ...
    last_pdd_start = last_pdd_execution_datetime - relativedelta(
        hour=0, minute=0, second=0, microsecond=0, days=1
    )
    month_delta = (
        1
...
def _charge_annual_fee(
    ...
    (annual_fee, fee_posting_instructions) = _charge_fee(
        vault, denomination, in_flight_balances=in_flight_balances, fee_type=ANNUAL_FEE
    )
...
def _process_statement_cut_off(
    ...
    if is_final:
        ...
        scod_cut_off_dt = scod_effective_dt - relativedelta(microseconds=1)
    ...
    supported_fee_types = _get_supported_fee_types(vault, supported_txn_types)
    accrue_interest_from_txn_day = _is_txn_interest_accrual_from_txn_day(vault)
    txn_types_with_params = utils_get_parameter(
        vault, name=PARAM_TXN_TYPES, at_datetime=effective_datetime, is_json=True
    )
    txn_types_to_charge_interest_from_txn_date = [
        txn_type
    ...
    instructions_ts[scod_cut_off_dt].extend(
        _charge_overlimit_fee(
            vault, in_flight_balances, denomination, supported_txn_types, credit_limit
        )
    )
    instructions_ts[scod_effective_dt].extend(
        _bill_charged_txns_and_bank_charges(
            vault, supported_txn_types, denomination, in_flight_balances, credit_limit
        )
    )
    ...
    mad = _calculate_mad(
        ...
        mad_eq_statement=utils_is_flag_in_list_applied(
            vault=vault,
    ...
    instructions_ts[scod_effective_dt].extend(
        _update_info_balances(vault, in_flight_balances, denomination, total_statement_amount, mad)
    )
    if not is_final:
...
def _bill_charged_txns_and_bank_charges(
    ...
    scod_instructions.extend(
        _bill_charged_interest(
            vault, supported_fee_types, supported_txn_types, denomination, in_flight_balances
        )
    )
    scod_instructions.extend(
        _adjust_aggregate_balances(
...
def _bill_charged_interest(
    ...
    for (txn_type, txn_refs) in supported_txn_types.items():
        for txn_ref in txn_refs or [""]:
            move_interest_to_billed(sub_type=txn_type, txn_ref=txn_ref)
    for fee_type in supported_fee_types:
...
def _update_info_balances(
    ...
    for info_balance in [STATEMENT_BALANCE, OUTSTANDING_BALANCE, FULL_OUTSTANDING_BALANCE]:
        info_balance_postings.extend(
            _override_info_balance(
                vault, in_flight_balances, info_balance, denomination, statement_amount
            )
        )
    info_balance_postings.extend(
        _override_info_balance(
            vault, in_flight_balances, TRACK_STATEMENT_REPAYMENTS, denomination, Decimal(0)
        )
    )
    info_balance_postings.extend(
...
def _clean_up_balance_inconsistencies(
    ...
    for address_stem in address_stems:
        ...
        from_amount = utils_balance_at_coordinates(
        ...
        if from_amount < 0:
            ...
            to_amount = utils_balance_at_coordinates(
...
def _create_statement_notification(
    vault: Any, statement_end: datetime, final_statement: Decimal, mad: Decimal, is_final: bool
) -> AccountNotificationDirective:
    """
    ...
    if not is_final:
        ...
        (current_pdd_start, current_pdd_end) = _get_next_pdd(
            payment_due_period, account_creation_dt, last_pdd_execution_datetime
        )
        current_pdd_execution_datetime = current_pdd_end
        (next_pdd_start, _) = _get_next_pdd(
            payment_due_period, account_creation_dt, current_pdd_execution_datetime
        )
...
def _override_info_balance(
    ...
    """
    current_balance = utils_balance_at_coordinates(
...
def _calculate_mad(
    ...
    """
    if statement_amount <= 0 or utils_is_flag_in_list_applied(
        vault=vault,
...
def _calculate_percentage_mad(
    ...
    def get_mad_component(
        ...
        unpaid = utils_balance_at_coordinates(
            balances=in_flight_balances, address=unpaid_address, denomination=denomination
        )
        billed = utils_balance_at_coordinates(
            balances=in_flight_balances, address=billed_address, denomination=denomination
        )
        return utils_round_decimal(percentage * (unpaid + billed), 2)
    ...
    for (txn_type, refs) in txn_types.items():
        ...
        for ref in refs or [""]:
            ...
            mad += get_mad_component(
                charge_type=INTEREST, sub_type=txn_type, percentage=interest_percentage, txn_ref=ref
            )
    for fee_type in fee_types:
...
def _charge_fee(
    ...
    if not is_external_fee:
        fee_amount = utils_get_parameter(name=fee_type.lower(), vault=vault)
    if fee_amount == 0:
        return (Decimal(0), [])
    income_account = _get_fee_internal_account(vault, fee_type, "", is_external_fee)
    fee_posting_instructions = _rebalance_fees(
        vault, fee_amount, denomination, in_flight_balances, income_account, fee_type
    )
    return (fee_amount, fee_posting_instructions)
...
def _get_fee_internal_account(
    ...
    if fee_type:
        if is_external_fee:
            fee_internal_account = utils_get_parameter(
...
def _charge_interest(
    ...
    if is_revolver:
        ...
        accruals_to_charge = {
            ...
            for (
                (charge_type, sub_type, accrual_type),
                accrual_amount,
            ) in accruals_by_sub_type.items()
        ...
        if is_pdd and (not charge_interest_free_period):
            ...
            instructions.extend(
                _make_accrual_posting(
                    ...
                    reverse=True,
                    instruction_details={
                        "description": f"Uncharged interest reversed for {txn_stem_with_accrual_type} - INTEREST_CHARGED"
                    },
                    accrual_type=accrual_type,
                )
            )
        ...
        for (ref, amounts) in ref_to_amount.items():
            if is_pdd or not _is_txn_type_in_interest_free_period(
                ...
                _charge_interest_per_txn_type(
                    charge_type, sub_type.upper(), amounts, txn_ref=ref, accrual_type=accrual_type
                )
...
def _process_payment_due_date(
    ...
    if not _is_revolver(in_flight_balances, denomination):
        instructions.extend(
            _change_revolver_status(vault, denomination, in_flight_balances, revolver=True)
        )
        accruals_by_sub_type: dict[tuple[str, str, str], dict[str, Decimal]] = {}
        for (dimensions, amount) in in_flight_balances.items():
    ...
    repayments = utils_balance_at_coordinates(
        balances=in_flight_balances, denomination=denomination, address=TRACK_STATEMENT_REPAYMENTS
    )
    mad = utils_balance_at_coordinates(
    ...
    if utils_is_flag_in_list_applied(
        ...
        (_, fee_posting_instructions) = _charge_fee(
            vault, denomination, in_flight_balances, LATE_REPAYMENT_FEE
        )
        instructions.extend(fee_posting_instructions)
        notification_directives.append(
            AccountNotificationDirective(
...
def _move_outstanding_statement(
    ...
    if not utils_is_flag_in_list_applied(
        vault=vault,
    ...
    if not utils_is_flag_in_list_applied(
        ...
        posting_instructions.extend(
            _move_outstanding_statement_balances_to_unpaid(
                vault, in_flight_balances, denomination, supported_txn_types, supported_fee_types
            )
        )
...
def _update_overdue_buckets(
    vault: Any, overdue_total: Decimal, in_flight_balances: BalanceDefaultDict, denomination: str
) -> list[CustomInstruction]:
    """
    ...
    for (overdue_bucket, amount) in new_overdue_buckets.items():
        instructions.extend(
            _override_info_balance(vault, in_flight_balances, overdue_bucket, denomination, amount)
        )
    return instructions
...
def _move_outstanding_statement_balances_to_unpaid(
    ...
    def move_statement_balance_to_unpaid(
        ...
        statement_to_past_due = utils_balance_at_coordinates(
...
def _move_funds_internally(
    ...
    return _create_custom_instructions(
        ...
        debit_address=debit_address,
        instruction_details={
            "description": f"Move balance from {debit_address} to {credit_address}"
        },
        credit_address=credit_address,
        in_flight_balances=in_flight_balances,
    )
...
def _reverse_uncharged_interest(
    ...
    def _execute_reversal(txn_type: str, txn_ref: Optional[str] = None) -> None:
        ...
        accrued_outgoing = utils_balance_at_coordinates(
            balances=in_flight_balances, address=address, denomination=denomination
        )
        if accrued_outgoing > 0:
            if txn_ref:
                if txn_type.endswith(INTEREST_FREE_PERIOD):
                    stem = f"{txn_type[:-(len(INTEREST_FREE_PERIOD) + 1)]}_{txn_ref}_{INTEREST_FREE_PERIOD}"
                else:
...
def _zero_out_mad_balance(
    vault: Any, mad_balance: Decimal, denomination: str
) -> list[CustomInstruction]:
    """
...
def _interest_address(
    ...
    """
    address_deconstructed = [txn_type.upper(), txn_ref, INTEREST, accrual_type, interest_status]
    if txn_ref and txn_type.upper().endswith(INTEREST_FREE_PERIOD):
        return f"{txn_type.upper()[:-(len(INTEREST_FREE_PERIOD) + 1)]}_{txn_ref}_{INTEREST_FREE_PERIOD}_{INTEREST}_{interest_status}"
...
def _charge_overlimit_fee(
    ...
    if overlimit_amount > 0 and opt_in:
        (fee, fee_instructions) = _charge_fee(
            vault, denomination, in_flight_balances, OVERLIMIT_FEE
        )
        if fee == 0:
            return []
        else:
...
def _get_non_advice_postings(
    ...
    return [
        ...
        or posting_instruction.advice is False
    ]
...
def _update_balances(
    account_id: str, balances: BalanceDefaultDict, posting_instructions: list[PostingInstruction]
) -> None:
    """
...
def _determine_txns_currently_interest_free(
    ...
    for txn_type in set.intersection(
        ...
        for ref in set.intersection(
            set(txn_base_interest_rates[txn_type]), set(txn_types_in_interest_free_period[txn_type])
        ):
...
def _is_txn_interest_accrual_from_txn_day(
    vault: Any, at_datetime: Optional[datetime] = None
) -> bool:
    """
...
def get_denomination_from_posting_instruction(posting_instruction: PostingInstruction) -> str:
    ...
    """
    return (
        posting_instruction.denomination
        if posting_instruction.type is not PostingInstructionType.CUSTOM_INSTRUCTION
        else posting_instruction.postings[0].denomination
    )
